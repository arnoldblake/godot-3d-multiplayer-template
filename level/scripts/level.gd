...
func _add_player(id: int, player_info : Dictionary) -> void:
	...
	var skin_enum: Character.SkinColor = player_info["skin"]
	rpc("sync_player_skin", id, skin_enum)
	
	rpc("sync_player_position", id, player.position)
	
func get_spawn_point() -> Vector3:
	var spawn_point: Vector2 = Vector2.from_angle(randf() * 2 * PI) * 10 # spawn radius
...
@rpc("any_peer", "call_local")
func sync_player_skin(id: int, skin_color: Character.SkinColor) -> void:
